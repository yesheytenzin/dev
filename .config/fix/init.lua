...
function M.refresh_projects_b4update()
    ...
    -- erase all projects from global config (function references should not be saved)
    ...
    -- erase our own project (function references shouldn't be saved)
    c_config.projects[cwd] = nil

    -- Merge tables while filtering out non-serializable data (functions)
    local function filter_non_serializable(tbl)
        local filtered = {}
    ...
    local complete_config = merge_tables(
        filter_non_serializable(HarpoonConfig),
        filter_non_serializable(expand_dir(c_config)),
        filter_non_serializable(expand_dir(current_p_config))
    )

    -- Ensure correct config, excluding any invalid data
    ensure_correct_config(complete_config)
